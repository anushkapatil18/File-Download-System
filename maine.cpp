...
int main() {
    ...
    key = contentServer.getFileKey("file1.txt");
    cout<<"Filekey for file1.txt: "<<key<<endl;
    fk = reinterpret_cast<unsigned char*>(key.data());
    encryptFile("file1.txt",fk);
    contentServer.addFile("file2.pdf", "password2");
    key = contentServer.getFileKey("file2.pdf");
    cout<<"Filekey for file2.pdf: "<<key<<endl;
    fk = reinterpret_cast<unsigned char*>(key.data());
    encryptFile("file2.pdf",fk);
    contentServer.addFile("file3.jpg", "password3");
    key = contentServer.getFileKey("file3.jpg");
   cout<<"Filekey for file3.jpg: "<<key<<endl;
    fk = reinterpret_cast<unsigned char*>(key.data());
    encryptFile("file3.jpg",fk);
    ...
    if (authenticationServer.authenticateUser(u)) {
        ...
        if (contentServer.isFileAvailable(filename)) {
            unsigned char serverFileKey[AES_KEY_SIZE/8];
		key = contentServer.getFileKey(filename);
		fileKey = reinterpret_cast<unsigned char*>(key.data());
    		cout<<"File key in vector: "<<contentServer.getFileKey(filename)<<endl;
    		cout<<"File key converted: "<<key<<endl;
            // Check that the client and server file keys match
            if (contentServer.getFilePassword(filename) == password) {
